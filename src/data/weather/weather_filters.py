...
def create_night_time_replace_handler(
    location: LocationInfo = LocationInfo("Bremen", "Germany", "Europe/Berlin", 53.0474, 8.786747),
    night_time_element_value: any = None,
):
    def exclude_night_time_values(
        ...
        for index, value in iter_timeseries(data):
            ...
            if not (last_sunrise <= DayTime.from_datetime(cur_py_datetime) <= last_sunset):
                res_data[index] = night_time_element_value
            # print(f"{index} => {res_data[index]}")

        return res_data

    return exclude_night_time_values
