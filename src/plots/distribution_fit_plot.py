...
from src.data.distribution.distribution_fit import (
    test_fit_against_all_distributions,
    create_pdf_series_from_distribution,
    DistributionFit,
)
...
__PLOT_DICT: dict[__Keys, dict[Locale, str]] = {
    __Keys.LEGEND_DATA_DEFAULT: {Locale.EN: "Data split into {X} bins", Locale.DE: "Daten aufgeteilt in {X} Klassen"},
    __Keys.Y_LABEL_DEFAULT: {Locale.EN: "Relative frequencies P(x)", Locale.DE: "Relative Häufigkeitsdichte P(x)"},
    __Keys.X_LABEL_DEFAULT: {Locale.EN: "Value", Locale.DE: "Wert"},
}
...
class DistributionPlotColumn:
    ...
    legend_spacing: Optional[tuple[float, float]] = None
...
def default_distribution_legend_label_provider(fit: DistributionFit, error_label: str) -> str:
    param_names = (
        (fit.distribution.shapes + ", loc, scale").split(", ") if fit.distribution.shapes else ["loc", "scale"]
    )
...
def draw_best_fit_plot(
    ...
    distribution_legend_label_provider_fn: Callable[
        [DistributionFit, str], str
    ...
    data.plot(
        kind="hist", bins=plot_metadata.bins, density=True, alpha=0.5, label=data_legend_label, legend=True, ax=ax
    )
    ...
    fitted_distributions = test_fit_against_all_distributions(
        data=data, bins=plot_metadata.bins, error_fn=error, distribution_names=distribution_names
    )
    ...
    if extra_plots is not None:
        for dist_name in extra_plots:
            dist = next(
                (
                    x
                    for x in fitted_distributions
                    if x.distribution_name == dist_name and x.distribution_name != best_fit.distribution_name
                ),
                None,
            )
            if dist is not None:
    ...
    ax.set_ylim(
        ...
        (
            data_ylim[1]
            if plot_metadata.legend_spacing is None
            else data_ylim[1] * (1 + (plot_metadata.legend_spacing[1] * legends_plotted))
        ),
    )
    ...
    ax.set_xlabel(
        translate(__Keys.X_LABEL_DEFAULT)
        if plot_metadata.plot_options.x_label is None
        else plot_metadata.plot_options.x_label
    )
    ax.set_ylabel(
        translate(__Keys.Y_LABEL_DEFAULT)
        if plot_metadata.plot_options.y_label is None
        else plot_metadata.plot_options.y_label
    )
    ax.legend(loc="upper right")

    return DistributionFitPlotResult(fits=fitted_distributions, best_fit=best_fit, plot_res=PlotResult(fig, ax))
