...
from src.data.weather.weather_dwd_importer import DWDWeatherDataImporter, WeatherDataColumns
...
from src.plots.zoom_line_plot import ZoomPlotOptions
...
def save_hist_plots_on_every_feature(
    ...
    def __draw_violin_plot_every_hour(
        ...
        sample_a_hours = np.mod((sample_a.index.hour.values + 2), sequence_len)
        sample_b_hours = np.mod((sample_b.index.hour.values + 2), sequence_len)
        tips = pd.DataFrame(
            {
    ...
    def __draw_qq_plot(
        ...
        if col == WeatherDataColumns.DH_W_PER_M2 or col == WeatherDataColumns.GH_W_PER_M2:
            sample_a = sample_a[sample_a != 0]
            sample_b = sample_b[sample_b != 0]
    ...
    def __draw_single_hist_plot(
        ...
        fig, ax = draw_hist_plot(
            [
                HistPlotData(data=sample_a, label=f"Generatorausgabe ({bins} Klassen)"),
                HistPlotData(data=sample_b, label=f"Trainingsdaten ({bins} Klassen)"),
            ],
            bins=bins,
            normalized=True,
            plot=plot,
    ...
    exclude_night_time_values = create_night_time_replace_handler(night_time_element_value=0)
    ...
    for col in dataframe.columns:
        ...
        if col_hist_options is not None:
            ...
            qq_fig, _ = __draw_qq_plot(
                sample_a=sample_a, sample_b=sample_b, col=col  # , PlotResult(multi_fig, multi_axes[col])
            )
            qq_fig.savefig(
...
def ks_test_on_every_feature(evaluator: Evaluator):
    ...
    for take in range(sample_count):
        ...
        for col in dataframe.columns:
            ...
            ttest_statistic, ttest_p_value = stats.ttest_ind(sample_a, sample_b, equal_var=False)
    ...
    latex_table += (
        r"\caption{Ergebnisse des KS-Tests und der Wasserstein-Metrik zwischen der Gesamtheit der Trainingsdaten $\mu$ von 11 Jahren und "
        + str(sample_count)
        + r" zufälligen Generatorausgaben $\nu_t$ eines nicht Schaltjahres unter Angabe des Mittelwertes und der Standardabweichung. Durch die Wahl von 11 Jahren und einem Jahr gilt $n="
        + str(ks_n)
        + r"$ und $m="
        + str(ks_m)
        + r"$"
